...
describe("Bansa", function () {
    ...
    before("init class", function () {
        ...
        console.log("mocWebClient : ", mocWebClient);
...
반사
`;
                expect(JSON.stringify(result)).to.be.eq(JSON.stringify({channel: channels.general, text, icon_emoji: ":raised_hand_with_fingers_splayed:"}));
                done();
            }).catch((error: any) => {
                console.error(error);
                done(error);
